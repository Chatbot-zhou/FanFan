...
fn run() -> Result<(), fanfan_core::AppError> {
    ...
    println!("== 逐题结果 ==");
    let mut chat_ok = 0_usize;
    let mut rag_ok = 0_usize;
    let mut find_ok = 0_usize;
    let mut no_evidence = 0_usize;
    let mut clarify = 0_usize;
    let mut bad = 0_usize;
    for (index, scenario) in scenarios.iter().enumerate() {
        let row = run_scenario(index, &catalog, &mut runtime, &ollama, &embed, &file_names, &cancelled, scenario)?;
        match row.branch {
            Branch::Chat => chat_ok += 1,
            Branch::Rag{..} => rag_ok += 1,
            Branch::Find => find_ok += 1,
            Branch::NoEvidence => no_evidence += 1,
            Branch::Clarify => clarify += 1,
        }
        if !row.ok {
            bad += 1;
            println!("  ❌ {:<4} [{:<14}] {}\n     branch={} note={}", row.id, row.category, row.question, row.branch_label, row.judgement);
        } else if row.verbose {
            println!("  ✅ {:<4} [{:<14}] {}\n     branch={}", row.id, row.category, row.question, row.branch_label);
        } else {
            println!("  ✅ {:<4} [{:<14}] {}", row.id, row.category, row.question);
        }
    ...
    println!(
        "\n== 汇总 ==\nchat={chat_ok} rag={rag_ok} find={find_ok} no_evidence={no_evidence} clarify={clarify} 判定不符={bad}"
    );
    Ok(())
}
...
fn run_scenario(
    ...
    let final_answer = if let Ok(generated) = generated
...
fn plan_label(plan: Option<&QueryPlan>) -> String {
    match plan {
        Some(plan) => format!(
            "intent={} op={} target={:?} content={:?}",
            plan.intent.as_str(),
            plan.operation.as_str(),
            plan.target
                .reference
                .as_deref()
                .or(plan.target.document_name.as_deref())
                .unwrap_or("-"),
