...
fn main() {
    ...
    for (label, path) in [("clean", clean), ("prefixed", prefixed)] {
        ...
        let tts = WorkerClient::from_executable(PathBuf::from(path))
            .clone()
            .with_role(WorkerRole::Speech);
        match tts.self_test_tts(
            r"C:\no\such\model.onnx".into(),
            r"C:\no\such\tokens.txt".into(),
            r"C:\no\such\lexicon.txt".into(),
            2,
        ) {
            Ok(result) => println!("[{label} speech.tts_self_test] OK {:?}", result),
            Err(error) => println!(
                "[{label} speech.tts_self_test] code={} (OPERATION_UNSUPPORTED=角色未生效) msg={}",
