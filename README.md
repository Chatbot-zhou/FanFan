<h1 align="center">翻翻 FanFan：完全本地化的 AI 信息管理助手</h1>

<p align="center">
  <img src="assets/logo.png" width="120" alt="翻翻 FanFan Logo"/>
</p>
...
- **核心特点**：本地运行、源文件只读、中文优先、可追溯、可离线。内置 SQLite/USearch 混合检索，生成与 Embedding 通过本机 Ollama 运行，资料无需上传。
...
| 首页 | 找资料 |
| --- | --- |
| ![翻翻首页：资料总览与快捷入口](assets/home.png) | ![找资料：混合检索本地资料](assets/search.png) |

| 智能集合 | 问资料 |
| --- | --- |
| ![智能集合：按主题自动归类整理](assets/collections.png) | ![问资料：基于本地资料的自然语言问答](assets/ask.png) |
...
3. **配置本地模型**：进入「设置 → 本地模型」，按角色配置本地模型：问答生成、多模态图片理解与 Embedding 由本机 Ollama tag 托管；Rerank、OCR、ASR 从已验证模型池下载并按大小与 SHA-256 校验后激活。
4. **找资料（搜索）**：在搜索页输入关键词，同时获得全文（SQLite FTS5）与语义（本地向量）的融合检索结果，结果支持快照分页、停止搜索与原文定位。
5. **问资料（问答）**：在问答页提出自然语言问题，翻翻基于检索到的本地资料生成可追溯回答，并锚定到原文引用；回答由本机 Ollama 本地服务完成，支持多模态图片理解。
6. **语音提问**：点击麦克风用语音提问（本地识别、识别结果确认后发送）；朗读/TTS 已移出 V1 主链。
...
- **可追溯问答**：本地检索增强生成，回答附带原文引用与锚定定位；通过本机 Ollama 本地服务生成，回答经结构化引用校验后展示
- **多模态图片理解**：扫描页、内嵌图与图表由本地视觉语言模型（VLM）补齐文字提取与摘要，搜索与问答可命中图片内容
- **本地语音**：ASR（SenseVoice/Paraformer + Silero VAD）语音提问由本地 sherpa-onnx 独立进程完成，临时音频不落盘；TTS/朗读已移出 V1 主链
- **多模态本地模型**：按角色管理生成、多模态（VLM）、Embedding、Rerank、OCR、ASR 语音识别模型；Ollama tag 与文件模型分别按各自契约管理
...
- **一键安装**：Windows x64 简体中文 NSIS 安装程序，内含 WebView2 离线安装器与独立 Worker；生成/Embedding 依赖本机 Ollama 服务

## 当前状态

翻翻处于体验版开发阶段（预发布版本 `0.2.0`），所有推理完全本地。截至 2026-08-22：

- **模型链路迁移中**：问答生成、多模态与 Embedding 主链已迁移到本机 Ollama；Rerank、OCR、ASR 仍使用本地文件模型并保留 SHA-256 校验。本机 Ollama 最小 chat/embed 探针已通过；拉取、停止、驻留、应用内问答与安装态验证仍在收口。
- **语音链路已接通**：ASR 模型与本地 sherpa-onnx 运行环境已就绪；应用内真实录音、识别与回填仍需最终体验验证。
- **图片理解全量处理中**：资料解析产生的全部图片资产正由本地 VLM 逐一补齐文字与摘要，完成后进入检索与问答索引。
- **真实资料评测进行中**：本地评测器正在对真实授权资料跑搜索、问答、关系、集合、OCR 与语音基线，全部通过并达到门禁分数前不发布综合评分。

## 贡献指南

欢迎通过 Issue、PR 参与翻翻的开发。请先阅读 `docs/翻翻-产品与开发规划书.md` 与 `docs/方案逐条审查清单.md` 了解产品方向。
