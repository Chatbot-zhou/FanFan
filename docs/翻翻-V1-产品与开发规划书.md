...
#### 6.3.1 ASR 与 TTS 模型：当前运行环境缺口与完成规划

**现状**（2026-08-14）：语音识别（`asr_sherpa_paraformer`：Paraformer 中文主模型 + tokens + Silero VAD 同一锁定预设）与语音生成（`tts_sherpa_vits`：sherpa-onnx VITS 中文）的**模型目录项、锁定下载、前端链路与 Worker 代码均已就绪**（`speech_recognize`/`speech_synthesize_answer` 命令、录音回填输入框、仅对通过引用校验的答案显示朗读按钮、`services/worker` 内 `recognize_speech`/`synthesize_speech`/`self_test_asr`/`self_test_tts` 实现），但**两个模型目前没有可运行的环境**：

- `services/worker` 依赖 `sherpa-onnx==1.13.4` 与 `sherpa-onnx-core`（内置 ONNX Runtime），当前开发机未安装该 Python 环境；
- 打包版 `fanfan-worker.exe`（PyInstaller sidecar，位于资源目录 `worker/`）尚未构建，`WorkerClient::from_executable` 走不到，ASR/TTS 推理无法在真实运行中启动；
- 结果：ASR/TTS 卡在「模型可下载、链路已接通、但推理环境缺失」，语音按钮相关能力处于不可用状态，也不应计入已验证能力。
