    ...
    <section className="page">
      ...
      <div className="settings-grid">
        ...
        <div className="settings-content">
          ...
          {tab === "models" && <>
            {(visibleDownloads.some(modelDownloadIsActive) || downloadSummary.attention_count > 0) && <section><h2>模型下载</h2><div className="setting-row model-download-overview"><div><strong>{downloadSummary.attention_count > 0 ? `${downloadSummary.attention_count} 项需要处理` : downloadSummary.active_count > 0 ? `${downloadSummary.active_count} 个任务正在进行` : `${downloadSummary.visible_count} 个任务已结束`}</strong><small>{downloadSummary.active_count > 0 ? `${downloadSummary.progress == null ? "正在准备下载" : `总体 ${Math.round(downloadSummary.progress * 100)}%`} · ${formatModelDownloadBytes(downloadSummary.downloaded_bytes)} / ${formatModelDownloadBytes(downloadSummary.total_bytes)}` : "完成和取消记录仅保留到本次应用会话结束"}</small></div><button type="button" onClick={() => navigate("model_setup")}>查看全部任务</button></div></section>}
            <section><h2>模型配置</h2><div className="setting-row"><div className="capability-list">{activeCapabilities === null ? "正在读取" : activeCapabilities.length > 0 ? activeCapabilities.join(" · ") : "尚未启用本地模型"}</div><button type="button" onClick={() => navigate("model_setup")}>管理模型</button></div></section>
