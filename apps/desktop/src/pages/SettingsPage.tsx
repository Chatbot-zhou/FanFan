...
import { ModelPresetPanel } from "../features/model-management/ModelPresetPanel";
import { OllamaRuntimePanel } from "../features/model-management/OllamaRuntimePanel";
...
import { cleanFullPath } from "../utils/display-path";
...
export function SettingsPage({ initialTab }: { initialTab?: SettingsTab }) {
  ...
  const cleanupStorageMigration = async () => {
    ...
    if (!previous) return;
    if (!await confirmAction({ actionKey: "storage_migration_cleanup", title: "清理迁移前的旧数据？", description: `将删除迁移前位于 ${cleanFullPath(previous)} 的全部旧数据（当前数据不受影响）。清理前会校验当前数据完整，校验通过才执行；删除后旧数据无法恢复。`, confirmLabel: "确认清理", danger: true, confirmPhrase: "CLEANUP_MIGRATED_STORAGE" })) return;
    setBusy(true); setError(null); setMessage(null);
    try {
  ...
  const cleanupModelStoreMigration = async () => {
    ...
    if (!previous) return;
    if (!await confirmAction({ actionKey: "model_store_migration_cleanup", title: "清理迁移前的旧模型仓库？", description: `将删除迁移前位于 ${cleanFullPath(previous)} 的全部旧模型（当前仓库不受影响）。清理前会校验当前仓库完整，校验通过才执行；删除后旧仓库无法恢复。`, confirmLabel: "确认清理", danger: true, confirmPhrase: "CLEANUP_MIGRATED_MODEL_STORE" })) return;
    setBusy(true); setError(null); setMessage(null);
    try {
      ...
      <div className="settings-grid">
        <aside className="settings-side">
          ...
          <nav className="settings-nav">
            <button type="button" className={tab === "roots" ? "active" : ""} onClick={() => setTab("roots")}>资料目录</button><button type="button" className={tab === "models" ? "active" : ""} onClick={() => setTab("models")}>模型管理</button><button type="button" className={tab === "index" ? "active" : ""} onClick={() => setTab("index")}>存储状态</button><button type="button" className={tab === "appearance" ? "active" : ""} onClick={() => setTab("appearance")}>外观显示</button><button type="button" className={tab === "memory" ? "active" : ""} onClick={() => setTab("memory")}>记忆</button><button type="button" className={tab === "logs" ? "active" : ""} onClick={() => setTab("logs")}>日志与恢复</button>
          </nav>
        </aside>
        <div className="settings-content">
          {tab === "index" && <section><h2>自适应存储空间</h2><div className="metric-strip"><span><strong>{storage.data ? bytes(storage.data.total_bytes) : "—"}</strong>当前占用</span><span><strong>{storage.data ? bytes(storage.data.soft_quota_bytes) : "—"}</strong>自适应配额</span><span><strong>{storage.data?.disk_available_bytes != null ? bytes(storage.data.disk_available_bytes) : "—"}</strong>磁盘可用</span></div>{storage.data?.notice && <p role="status" className="inline-error">{storage.data.notice}</p>}<small>应用管理目录：{cleanFullPath(storage.data?.data_directory ?? "正在读取")}</small></section>}
          {tab === "index" && <section><h2>迁移应用数据</h2><p>可把数据库、USearch索引和缓存迁移到其他本地磁盘。已下载模型位于独立仓库，不随数据库重置或应用数据迁移而删除。</p><div className="readonly-note">{storageLocation.data?.pending_target_directory ? <><span>迁移前：{cleanFullPath(storageLocation.data?.active_data_directory ?? storage.data?.data_directory ?? "正在读取")}</span><span>迁移后：{cleanFullPath(storageLocation.data.pending_target_directory)}</span></> : <>当前位置：{cleanFullPath(storageLocation.data?.active_data_directory ?? storage.data?.data_directory ?? "正在读取")}</>}</div>{storageLocation.data?.previous_data_directory && !storageLocation.data?.pending_target_directory && <p className="cleanup-hint">迁移前的旧数据仍保留于 {cleanFullPath(storageLocation.data.previous_data_directory)}，确认新位置正常后可清理以释放磁盘空间；清理后即可安排新的迁移。</p>}{storageLocation.data?.last_error && <p role="alert" className="inline-error">{storageLocation.data.last_error}</p>}<div className="settings-actions"><button type="button" disabled={busy || (storageLocation.data?.restart_required && !storageLocation.data?.last_error) || !!storageLocation.data?.previous_data_directory} onClick={() => void scheduleStorageMigration()}><FolderOpenOutlined /> {storageLocation.data?.restart_required && !storageLocation.data?.last_error ? "等待重启迁移" : storageLocation.data?.restart_required ? "重新选择迁移位置" : "选择新位置"}</button>{storageLocation.data?.previous_data_directory && !storageLocation.data?.pending_target_directory && <button type="button" className="danger-button" disabled={busy} onClick={() => void cleanupStorageMigration()}><DeleteOutlined /> 确认清理旧数据</button>}</div></section>}
          {tab === "index" && <section><h2>迁移模型</h2><p>可把已下载模型仓库迁移到其他本地磁盘，与应用数据分开存储。重启后生效。</p><div className="readonly-note">{modelStore.data?.pending_target_directory ? <><span>迁移前：{cleanFullPath(modelStore.data?.store_path ?? "正在读取")}</span><span>迁移后：{cleanFullPath(modelStore.data.pending_target_directory)}</span></> : <>当前位置：{cleanFullPath(modelStore.data?.store_path ?? "正在读取")}{modelStore.data ? ` · ${modelStore.data.installed_artifacts} 个组件 / ${bytes(modelStore.data.installed_bytes)}` : ""}</>}</div>{modelStore.data?.previous_model_store && !modelStore.data?.pending_target_directory && <p className="cleanup-hint">迁移前的旧模型仓库仍保留于 {cleanFullPath(modelStore.data.previous_model_store)}，确认新位置正常后可清理以释放磁盘空间；清理后即可安排新的迁移。</p>}{modelStore.data?.last_error && <p role="alert" className="inline-error">{modelStore.data.last_error}</p>}<div className="settings-actions"><button type="button" disabled={busy || (modelStore.data?.restart_required && !modelStore.data?.last_error) || !!modelStore.data?.previous_model_store} onClick={() => void scheduleModelStoreMigration()}><FolderOpenOutlined /> {modelStore.data?.restart_required && !modelStore.data?.last_error ? "等待重启迁移" : modelStore.data?.restart_required ? "重新选择迁移位置" : "选择新位置"}</button>{modelStore.data?.previous_model_store && !modelStore.data?.pending_target_directory && <button type="button" className="danger-button" disabled={busy} onClick={() => void cleanupModelStoreMigration()}><DeleteOutlined /> 确认清理旧模型仓库</button>}</div></section>}
          {tab === "roots" && <>
