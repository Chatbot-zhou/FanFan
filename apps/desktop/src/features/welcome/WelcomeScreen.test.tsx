...
describe("WelcomeScreen", () => {
  ...
  it("shows no start button on the first screen", () => {
    ...
    expect(screen.getByText("想不起来？翻翻就知道。")).toBeInTheDocument();
  ...
  it("allows a blank-area click to reveal the second screen exactly once", () => {
    ...
    expect(screen.getByText("想得到，搜得到，翻翻知道。")).toBeInTheDocument();
  ...
  it("persists completion before entering the main application", async () => {
    ...
    expect(JSON.parse(window.localStorage.getItem("fanfan.welcome.v1") ?? "{}").welcome_completed).toBe(true);
  });
});
