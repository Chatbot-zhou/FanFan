...
export function StatusBar({ snapshot }: StatusBarProps) {
  ...
  return (<>
    ...
    <Drawer placement="bottom" height="min(340px, 52vh)" open={open} onClose={() => setOpen(false)} closable={false} className="app-status-drawer">
      ...
      <div className="app-status-panel">
        <section className="app-status-panel__coverage"><header><h3>检索覆盖</h3><span>{embeddingCoverage}%</span></header><Progress percent={embeddingCoverage} size="small" showInfo={false} strokeColor="#7468cf" trailColor="rgba(116,104,207,.13)" /></section>
        <div className="app-status-panel__compact-grid">
          <section><header><h3>授权资料位置</h3><span>{enabledRoots ?? 0} 个</span></header><div className="status-root-list">{roots?.filter((root) => root.enabled).slice(0, 3).map((root) => <div key={root.root_id}><span className={`status-dot status-dot--${root.status}`} /><strong>{root.label}</strong><small>{root.status === "ready" ? "在线" : root.status === "scanning" ? "扫描中" : root.status === "offline" ? "离线" : "需检查"}</small></div>)}{(enabledRoots ?? 0) === 0 && <p>尚未授权资料位置</p>}</div></section>
