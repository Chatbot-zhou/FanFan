...
describe("model download view model", () => {
  it("keeps creation order when updated_at alternates", () => {
    ...
    expect(visibleModelDownloadJobs([second, first]).map((item) => item.job_id)).toEqual(["first", "second"]);
    ...
    expect(visibleModelDownloadJobs([nextSecond, nextFirst]).map((item) => item.job_id)).toEqual(["first", "second"]);
  });
  ...
  it("keeps failures and hides terminal success or cancellation", () => {
    ...
    expect(visibleModelDownloadJobs([previousCompletion, currentCancellation, previousFailure]).map((item) => item.job_id)).toEqual(["failed"]);
  });
});
